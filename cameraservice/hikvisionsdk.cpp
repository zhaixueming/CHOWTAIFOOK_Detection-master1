...
namespace vimo::display
{
	...
	HikvisionSDK::HikvisionSDK(QString camName) :m_camName(camName)
	{
		m_hDevHandle = NULL;
	}
	...
	// 注册图像数据回调
	int HikvisionSDK::RegisterImageCallBack(void(__stdcall* cbOutput)(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo, void* pUser))
	{
		if (NULL == m_hDevHandle)
		{
			return -1;
		}
		return MV_CC_RegisterImageCallBackEx(m_hDevHandle, cbOutput, this);
	}

	// 注册消息异常回调
	int HikvisionSDK::RegisterExceptionCallBack(void(__stdcall* cbException)(unsigned int nMsgType, void* pUser))
	{
		void* pUser = nullptr;
		if (NULL == m_hDevHandle)
		{
	...
	// 设置显示窗口句柄
	int HikvisionSDK::Display(void* hWnd)
	{
		if (NULL == m_hDevHandle)
		{
			return -1;
	...
	int HikvisionSDK::SaveImage(MV_SAVE_IMAGE_PARAM_EX* pstParam)
	{
		if (NULL == pstParam)
		{
			return -1;
	...
	//获取相机增益
	float HikvisionSDK::getGain()
	{
		MVCC_FLOATVALUE stParam;
		memset(&stParam, 0, sizeof(MVCC_INTVALUE));
		int tempValue = MV_CC_GetFloatValue(m_hDevHandle, "Gain", &stParam);
		float value = stParam.fCurValue;
		if (tempValue != 0)
		{
			return -1;
	...
	//设置增益
	int HikvisionSDK::setGain(float GainNum)
	{
		int tempValue = MV_CC_SetFloatValue(m_hDevHandle, "Gain", GainNum);
		if (tempValue != 0)
		{
			return -1;
		}
	...
	//清理相机缓存
	void HikvisionSDK::clearCamera()
	{
		stopCamera();
		closeCamera();
	}
	//发送取图信号
	void HikvisionSDK::emitSignall(const cv::Mat & image)
	{
		Mat imageshow = image.clone();
		Mat imagesave = image.clone();
		emit imageshowsignall(imageshow);
		emit imagesaveignall(imagesave);
		emit imageoperationignall(image);	
	}
	void HikvisionSDK::emitSignalml(const cv::Mat & image)
	{
		Mat imageshow = image.clone();
		Mat imagesave = image.clone();
		emit imageshowsignalml(imageshow);
		emit imagesaveignalml(imagesave);
		emit imageoperationignalml(image);		
	}
	void HikvisionSDK::emitSignalmr(const cv::Mat & image)
	{
		Mat imageshow = image.clone();
		Mat imagesave = image.clone();
		emit imageshowsignalmr(imageshow);
		emit imagesaveignalmr(imagesave);
		emit imageoperationignalmr(image);		
	}
	void HikvisionSDK::emitSignalr(const cv::Mat & image)
	{
		Mat imageshow = image.clone();
		Mat imagesave = image.clone();
		emit imageshowsignalr(imageshow);
		emit imagesaveignalr(imagesave);
		emit imageoperationignalr(image);
	}
}
